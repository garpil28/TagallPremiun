...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    print(f"DEBUG: /start dari user {user_id}")
    if user_id != OWNER_ID:
        await update.message.reply_text("❌ Kamu bukan owner bot. Hubungi admin.")
        return
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user_id = update.effective_user.id
    print(f"DEBUG: Pesan masuk dari {user_id}: {text}")
    ...
    if context.user_data.get("waiting_for") == "partner_list":
        ...
        for chat_id in lines:
            c.execute("INSERT OR REPLACE INTO partners(chat_id,is_partner,last_tag_date) VALUES(?,?,?)",
                      (chat_id,1,None))
