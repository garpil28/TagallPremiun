...
client = MongoClient(CBOT.DATABASE_URL)
db = client["garfield_tagall"]
partners = db.partners

async def auto_tagall():
    while True:
        now = datetime.now().strftime("%H:%M")
        if now == "09:00":  # waktu otomatis jalan
            for patner in partners.find({"used_today": {"$ne": True}}):
                chat_id = patner["chat_id"]
                text = f"👋 Halo {patner['name']}, ini tagall otomatis harian!"
                try:
                    await app.send_message(chat_id, text)
                    partners.update_one({"chat_id": chat_id}, {"$set": {"used_today": True}})
                    print(f"[AUTO] Tagall sent to {chat_id}")
                except Exception as e:
                    print(f"[ERROR] Gagal kirim ke {chat_id}: {e}")
        await asyncio.sleep(60)

asyncio.get_event_loop().create_task(auto_tagall())
